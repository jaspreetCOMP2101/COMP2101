...
echo "$1 is invalid. error occuredd."
}
...
function error-exit {
error-message >&2;
return 1 #it will not close shell
}
#This function displays help information if the user asks for it on the command line or gives us a bad command line
function displayhelp {
  echo "--host will display hostname"
  echo "--domain will display domain name"
  echo "--ipconfig will display ip informations"
  echo "--os will display operating system informations"
  echo "--cpu will display cpu informations"
  echo "--memory will display memory informations"
  echo "--disk will display disk informations"
  echo "--printer will display printer informations"
}
...
function cleanup {
echo "Interrupt execution started."
rm -fr /tmp/*info*
}
trap cleanup SIGINT
